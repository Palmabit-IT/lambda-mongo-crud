...
module.exports = {
  ...
  getProjections: function (query) {
    ...
    if (typeof query.projections === 'string') {
      try {
        query.projections = query.projections.split(",");
      } catch (e) {
        console.log('error in parsing projections', e);
        return project
      }
    }

    if (Array.isArray(query.projections)) {
      query.projections.map(proj => {
        project[proj] = 1
      })
    }
